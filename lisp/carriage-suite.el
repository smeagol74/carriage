...
(defun carriage--suite-guardrails (ops)
  ...
  (let* ((allowed (mapconcat (lambda (o) (format "%s" o)) ops ", "))
         (lines (list
                 ...
                 "- HEADER FORMAT (CRITICAL — STEP 0): #+begin_patch (:op \"create\" :version \"1\" :file \"path.el\")"
                 "- Header MUST be a single-line S-expression in parentheses on the #+begin_patch line."
                 "- NEVER put :op/:version/:file/:path on separate lines after #+begin_patch."
                 "- WRONG (INVALID — REJECT AND REGENERATE):\n  #+begin_patch\n  :op \"create\"\n  :version \"1\"\n  :file \"path.el\"\n  #+end_patch"
                 "- FORMAT CHECK (step 0): If header spans multiple lines with params on separate lines -> INVALID."
                 "- STRATEGY NOTE: For complex edits (>50% of file content), prefer :op delete + :op create over :op aibo/patch."
                 "- Delete+create is cleaner than heavily fragmented patches."
                 "1) CREATE new file (user explicitly asks + path NOT in begin_map):\n"
                 "   - ABSOLUTE: use :op create immediately. NEVER request begin_context for files to create.\n"
                 "   - Hard rule: absence in begin_map + explicit create = MUST create.\n"
                 "   - FORMAT EXAMPLE (CORRECT):\n     ;; patch history: config/new-file.js — (no description)\n     #+begin_patch (:op \"create\" :version \"1\" :file \"config/new-file.js\")\n     ;; file content here\n     #+end_patch"
                 ...
                 "\n"
                 "PATCH HEADER VALIDATION (CRITICAL):\n"
                 "- CORRECT: #+begin_patch (:op \"aibo\" :version \"1\" :file \"path.el\")\n"
                 "- INVALID (REJECT):\n  #+begin_patch\n  :op \"aibo\"\n  :version \"1\"\n  #+end_patch"
                 "- Tool will reject multi-line headers. Always use single-line S-expression."
                 "\n"
                 "PATCH HEADER VALIDATION (CRITICAL):\n"
                 "- CORRECT: #+begin_patch (:op \"aibo\" :version \"1\" :file \"path.el\")\n"
                 "- INVALID (REJECT):\n  #+begin_patch\n  :op \"aibo\"\n  :version \"1\"\n  #+end_patch"
